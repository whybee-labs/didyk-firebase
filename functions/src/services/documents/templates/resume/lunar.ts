...
export function render(doc: PDFKit.PDFDocument, data: Record<string, unknown>): void {
  ...
  const nameSize = autoFitText(doc, nameStr, "Inter-Bold", 18, 12, boxW - 24, 1.5);
  doc.font("Inter-Bold").fontSize(nameSize).fillColor(NAVY);
  ...
  if (d.github) detailPairs.push({ label: "GitHub", value: d.github });
  ...
  renderOptionalSections(doc, d, M, W,
    (title) => { pageBreak(doc, 40); doc.font("Inter-SemiBold").fontSize(10).fillColor(NAVY).text(title.toUpperCase(), M, doc.y, { width: W }); hr(doc, M, doc.y + 2, W, NAVY, 0.6); doc.y += 8; },
    { title: "Inter-SemiBold", body: "Inter", meta: "Inter-Italic", bullet: "Inter" },
