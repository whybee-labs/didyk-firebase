...
export function render(doc: PDFKit.PDFDocument, data: Record<string, unknown>): void {
  ...
  // ── Header: name + role ──
  const nameSize = autoFitText(doc, d.fullName, "Inter-Bold", 22, 14, FULL_W);
  doc.font("Inter-Bold").fontSize(nameSize).fillColor("#111111")
    .text(d.fullName, M, 28, { width: FULL_W });
  doc.font("Inter").fontSize(10).fillColor("#555555")
    .text(d.targetRole, M, doc.y + 3, { width: FULL_W });

  const ROW_START = doc.y + 16;
  ...
  const contactItems = [d.email, d.phone, d.linkedin, d.github, d.website, d.address].filter(Boolean) as string[];
  if (contactItems.length) {
    ...
    for (const c of contactItems) {
      renderContactItem(doc, c, DETAIL_X, rightY, DETAIL_W, "Inter", 8, "#555555");
      rightY = doc.y + 4;
    }
  }
  ...
  renderOptionalSections(doc, d, M, FULL_W,
    (title) => { pageBreak(doc, 40); doc.font("Inter-SemiBold").fontSize(9).fillColor(ACCENT).text(title, M, doc.y, { width: FULL_W }); hr(doc, M, doc.y + 2, FULL_W, ACCENT, 0.8); doc.y += 8; },
    { title: "Inter-SemiBold", body: "Inter", meta: "Inter-Italic", bullet: "Inter" },
